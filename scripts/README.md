...
- `build.py`: fully offline once `upstream/` is populated. Builds TTF and WOFF2 versions of every upright face by default and every italic face with `CJK_PUNCT_ITALIC=1`, with SC as the no-language default, corresponding Noto regional CJK paths, explicit Western-language Hanken punctuation, and Zhudou Chinese dashes.
...
- `build_interrobang_bridge.py`: builds the independent Inter-only companion with `!`, `?`, `¡`, `¿`, `！`, `？`, and `U+203D` in cmap, contextual 500+0/1000+0 pair shaping, nine static weights per posture, and two variable fonts.
...
`SOURCES.md` records the immutable source revisions and hashes used by v1.3.4. All Noto Sans and Hanken Grotesk inputs are pinned Google Fonts repository files.
