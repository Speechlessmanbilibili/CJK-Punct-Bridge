...
- `build_interrobang_bridge.py`: builds the independent Inter-only Western companion with exactly `!`, `?`, and `U+203D` in cmap, default `?!`/`!?` ligatures, nine static weights per posture, and two variable fonts.
...
`SOURCES.md` records the immutable source revisions and hashes used by v1.3.3. All Noto Sans and Hanken Grotesk inputs are pinned Google Fonts repository files.
